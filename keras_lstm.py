...
data = create_lagged_variables(data, 'caiso_load_actuals', lag_range=7) # Create lagged variables
...
X = df.drop(['datetime','caiso_load_actuals'], axis=1).values
y = df['caiso_load_actuals'].values
...
# Calculate MAPE using the predicted and actual values
mape = np.abs(y_test_inv - y_pred_actual) / np.abs(y_test_inv) * 100
